...
export async function POST(req: Request) {
  try {
    ...
    const safeWorkerProfile = workerProfile;
    ...
    const { data: existingBooking } = await adminSupabase
      ...
      .eq("worker_profile_id", safeWorkerProfile.id)
    ...
    const bookingCountry =
      country ||
      safeWorkerProfile.country ||
      workerUserProfile.country ||
      "United Kingdom";

    const bookingRegion =
      region ||
      safeWorkerProfile.region ||
    ...
    const bookingPostcodeFull =
      postcodeFull ||
      postcode ||
      safeWorkerProfile.postcode_full ||
      safeWorkerProfile.postcode ||
    ...
    const bookingPostcodePrefix =
      postcodePrefix ||
      safeWorkerProfile.postcode_prefix ||
      workerUserProfile.postcode_prefix ||
      hirerProfile.postcode_prefix ||
      null;

    const bookingAreaSlug =
      areaSlug || safeWorkerProfile.area_slug || "united-kingdom";
    ...
    const insertPayload: Record<string, unknown> = {
      ...
      worker_profile_id: safeWorkerProfile.id,
      title,
      message,
